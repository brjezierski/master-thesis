...
class DistilLoss(nn.Module):
    ...
    def __init__(self,
        ...
        if bias_inits is None:
            ...
            biases = torch.tensor(
                bias_inits, requires_grad=True, dtype=torch.float32)
            self.score_bias = nn.Parameter(biases)
    ...
    def forward(self, sentence_features: Iterable[Dict[str, Tensor]], labels: Tensor):
        ...
        outputs = self.score_bias.to(
            self.model._target_device) * outputs.to(self.model._target_device)
...
class MultipleConsistencyLoss(nn.Module):
    ...
    def forward(self, sentence_features: Iterable[Dict[str, Tensor]], labels: Tensor):
        ...
        if self.training_mode:
            teacher_reps = [self.teacher(sentence_feature)[
            ...
            teacher_scores = self.similarity_fct(
                teacher_embeddings_a, teacher_embeddings_b) * self.scale
        else:
            similarity_labels = labels[:, -1]
            teacher_scores = similarity_labels * self.scale
...
class DistilLoss2(nn.Module):
    """
    Residual part will be global similarity
    Distill metrics, Decompose output space
    ...
    def __init__(self,
                 ...
                 loss_fct: Callable = nn.MSELoss(),
                 sim_fct: Callable = dist_sim):
        super(DistilLoss2, self).__init__()
        self.model = model
        self.training_mode = training_mode
        self.num_labels = num_labels
        self.sentence_embedding_dimension = sentence_embedding_dimension
    ...
    def forward(self, sentence_features: Iterable[Dict[str, Tensor]], labels: Tensor):
        ...
        # print(sentence_features[0]['input_ids'].size())
        reps = [self.model(sentence_feature)['sentence_embedding']
                for sentence_feature in sentence_features]
        rep_a, rep_b = reps
        # print('rep_a',rep_a.size())
