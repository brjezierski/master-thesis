...
from clustering_utils import get_overlap_loss, get_diversity_loss
from clustering_utils import get_bert_topics, get_chat_intents_topics
import random
import collections
...
class ChatIntents:
    def __init__(self, training_embeddings, inference_embeddings, training_df, inference_df, name):
        self.training_embeddings = training_embeddings
        self.inference_embeddings = inference_embeddings
        ...
        Arguments:
            training_embeddings: numpy array of document embeddings for training of the topic model
            inference_embeddings: numpy array of document embeddings for infering from the topic model
    ...
    def generate_clusters(self,
        ...
        umap_model = umap.UMAP(n_neighbors=n_neighbors,
                               ...
                               metric='cosine',
                               random_state=random_state).fit(self.training_embeddings)
        umap_embeddings = umap_model.transform(self.inference_embeddings)
    ...
    def _objective(self, space, params, label_lower, label_upper, metric='outliers', df=None):
        """
    ...
    def plot_best_clusters(self, n_neighbors=15, min_dist=0.1):
        ...
        umap_reduce = (umap.UMAP(n_neighbors=n_neighbors,
                                 ...
                                 # metric='cosine',
                                 random_state=42)
                       .fit_transform(self.inference_embeddings)
                       )

        point_size = 100.0 / np.sqrt(self.inference_embeddings.shape[0])
